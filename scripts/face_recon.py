...
def get_data_path(root, keypoint_root, dataset_name):
    assert dataset_name in ['ffhq', 'celeba']
    filenames = list()
    ...
    if dataset_name == 'celeba':
        for ext in extensions:
            filenames += glob.glob(f'{root}/*/images/*.{ext}', recursive=True)     
        filenames = sorted(filenames)
        for filename in filenames:
            name = os.path.splitext(os.path.basename(filename))[0]
            name = name.zfill(5)
            val_train_set = 'train' if 'train' in filename else 'test'
            keypoint_filenames.append(
                os.path.join(keypoint_root, val_train_set, name + '.txt')
            )  
...
def main(opt, model):
    ...
    filenames, keypoint_filenames = get_data_path(opt.input_dir, opt.keypoint_dir, opt.dataset_name)
    ...
    savemat(os.path.join(opt.output_dir, 'semantic.mat'), {'coeff':pred_coeffs, 'transform_params':pred_trans_params})
