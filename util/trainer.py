...
from util.distributed import get_world_size
...
def accumulate(model1, model2, decay=0.999):
    ...
    buff1 = dict(model1.named_buffers())
    buff2 = dict(model2.named_buffers())

    for k in buff1.keys():
        if 'running_mean' in k or 'running_var' in k:
            buff1[k].data.mul_(decay).add_(buff2[k].data, alpha=1 - decay)    
        else:
            buff1[k].copy_(buff2[k].data)
...
def get_model_optimizer_and_scheduler(opt):
    ...
    # custom_opts = _get_custom_optimizers(getattr(opt, "custom_optimizers", None), net_G)
