...
def tensor_to_pilimage(image_tensor, norm=True):
    if len(image_tensor.shape) == 4:
        img = []
        for item in image_tensor:
            img.append(tensor_to_pilimage(item))
        return img
    else:
        if norm:
            image_tensor = (image_tensor+1.)/2.
        image_tensor = image_tensor.clamp_(0, 1).permute(1,2,0)
        image_numpy = (image_tensor.detach().cpu().numpy()*255).astype(np.uint8)
        image_plt = Image.fromarray(image_numpy)
        return image_plt
...
def save_pilimage_in_png(fullname, output_img):
    dirname = os.path.dirname(fullname)
    os.makedirs(dirname, exist_ok=True)
    output_img.save(fullname)
