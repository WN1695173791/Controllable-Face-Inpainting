...
class BaseTrainer(object):
    ...
    def save_checkpoint(self, current_epoch, current_iteration):
        ...
        _save_checkpoint(self.opt,
                         self.net_G, self.net_G_ema, self.net_D,
                         self.opt_G, self.sch_G, self.opt_D, 
                         self.sch_D, current_epoch, current_iteration)
    ...
    def _load_checkpoint(self, checkpoint_path, resume=True):
        ...
        if self.opt.phase == 'train' and resume:
            # the provided checkpoints do not contains these parameters
            # use --no_resume for continue training
            ...
            print('load [optimizers], [schdules], and [discriminator] from {}'.format(checkpoint_path))
    ...
    def test(self, data, output_dir, current_iteration):
...
def _save_checkpoint(opt,
                     net_G, net_G_ema, net_D, opt_G, sch_G, opt_D, sch_D,
